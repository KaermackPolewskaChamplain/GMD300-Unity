...
public class ThirdPersonControllerWithCinemachine : MonoBehaviour
{
    ...
    private void Update()
    {
        if (canControl)
        {
            ...
            //Only rotate when moving, not when the camera recenters itself
            if(currentHorizontalVelocity.magnitude > 0.001)
            {
                RotateCharacter();
            }
    ...
    private void CalculateHorizontalMovement()
    {
        ...
        //This adjust the moveInput onto the camera forward direction. We ignore the Y axis as we don't want to modify upward velocity, only horizontal.
        Vector3 cameraOrientedTargetDirection = new Vector3(moveInput.x, 0, moveInput.y);
        cameraOrientedTargetDirection = AlignVectorToCameraDirection(playerCamera, cameraOrientedTargetDirection);
        Vector2 cameraOrientedHorizontalDirection = new Vector2(cameraOrientedTargetDirection.x, cameraOrientedTargetDirection.z);
        ...
        currentHorizontalVelocity = Vector2.Lerp(currentHorizontalVelocity, cameraOrientedHorizontalDirection * targetSpeed, Time.deltaTime * targetAcceleration);
    }
    ...
    private Vector3 AlignVectorToCameraDirection(Camera camera, Vector3 worldSpaceVector)
    {
        Vector3 cameraSpaceVector = camera.transform.TransformDirection(worldSpaceVector);
